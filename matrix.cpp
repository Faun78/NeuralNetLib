...
mt19937_64 mt2(time(0));
static std::uniform_real_distribution<> dis00(0,1);
class Matrix{
    public:
        int rows;
        int cols;
        ...
        void mutate1(int min =0,int max=10){
            for(int i=0;i<rows;i++){
                for(int j=0;j<cols;j++){
                    double randNum=10;
                    static std::uniform_real_distribution<> dis(-5, 5); // range -1 - 1
                    while(randNum<-5 || randNum>5){
                    randNum =dis(mt2);
                    //randNum=randNum/1000000000;
                }
                    data[i][j]=randNum;
                }}}
